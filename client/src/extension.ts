...
export function activate(context: vscode.ExtensionContext) {
    context.subscriptions.push(vscode.languages.registerDocumentSymbolProvider(
        {language: "netdata"}, new NetdataDocumentSymbolProvider()
    ));
    context.subscriptions.push(vscode.languages.registerDefinitionProvider(
        {language: "netdata"}, new NetdataDefinitionProvider()
    ));
}
    ...
    public provideDocumentSymbols(document: vscode.TextDocument,
        ...
        return new Promise((resolve, reject) => {
            ...
            function findClosingBracketMatchIndex(startRow, pos) {
                ...
                let p= pos
                rowLoop:
                for (let i = startRow; i < document.lineCount; i++) {
                    let str = document.lineAt(i).text;
                    colLoop:
                    while (p < str.length) {
                        let openPos=str.indexOf('{', p)
                        let closePos=str.indexOf('}', p)
                        if (openPos != -1 && (closePos== -1  || openPos < closePos)) { //nasiel som otvaraciu & bud neexistu uzatvaracia nasleduje neskor
                            //console.log('Pozor: koment block na riadku: '+ i+1 +'  stlpci:'+openPos+1+ '"' + str+ '"' )
                            // console.log('open: '+openBracketCount+' ['+ i +', '+openPos+ '] "' + str+ '"' )
                            openBracketCount++;
                            p = openPos;
                        } else if (closePos != -1  ) {  // nasiel som uzatvaraciu
                            if (openBracketCount === givenBracketPosition) {  // som na urovni givenBracketPosition=1, mam co som chcel
                                indexC = closePos;
                                indexR = i;
                                break rowLoop;
                            }
                ...
                if(indexC == -1) { i = document.lineCount; indexC=0;}
